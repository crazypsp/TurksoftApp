...
// UrunTipi adındaki Entity (veritabanı tablosunu temsil eden sınıf).
...
namespace TurkSoft.Service.Manager
{
    ...
    // IUrunTipiService arayüzünü (interface) implement ederek, bu interface'de tanımlanan tüm metodları somut olarak uygular.
    public class UrunTipiManager : IUrunTipiService
    {
        ...
        private readonly IBaseService<UrunTipi> _baseBusiness;

        // Constructor (yapıcı metod), dışarıdan bu sınıfa IBaseService<UrunTipi> türünde bir bağımlılık (dependency) enjekte eder.
        // Dependency Injection (DI) prensibi sayesinde loosely-coupled (gevşek bağlı) bir yapı elde edilir.
        public UrunTipiManager(IBaseService<UrunTipi> baseBusiness)
        {
        ...
        public async Task<UrunTipi> AddAsync(UrunTipi entity)
        {
        ...
        public async Task<List<UrunTipi>> GetAllAsync()
        {
        ...
        public async Task<UrunTipi> GetByIdAsync(Guid id)
        {
        ...
        public async Task<UrunTipi> UpdateAsync(UrunTipi entity)
        {
