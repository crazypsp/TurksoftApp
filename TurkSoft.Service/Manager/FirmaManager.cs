...
// Firma adındaki Entity (veritabanı tablosunu temsil eden sınıf).
...
namespace TurkSoft.Service.Manager
{
    ...
    // IFirmaService arayüzünü (interface) implement ederek, bu interface'de tanımlanan tüm metodları somut olarak uygular.
    public class FirmaManager : IFirmaService
    {
        ...
        private readonly IBaseService<Firma> _baseBusiness;

        // Constructor (yapıcı metod), dışarıdan bu sınıfa IBaseService<Firma> türünde bir bağımlılık (dependency) enjekte eder.
        // Dependency Injection (DI) prensibi sayesinde loosely-coupled (gevşek bağlı) bir yapı elde edilir.
        public FirmaManager(IBaseService<Firma> baseBusiness)
        {
        ...
        public async Task<Firma> AddAsync(Firma entity)
        {
        ...
        public async Task<Firma> GetByIdAsync(Guid id)
        {
        ...
        public async Task<Firma> UpdateAsync(Firma entity)
        {
