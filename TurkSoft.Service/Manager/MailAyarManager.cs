...
// MailAyar adındaki Entity (veritabanı tablosunu temsil eden sınıf).
...
namespace TurkSoft.Service.Manager
{
    ...
    // IMailAyarService arayüzünü (interface) implement ederek, bu interface'de tanımlanan tüm metodları somut olarak uygular.
    public class MailAyarManager : IMailAyarService
    {
        ...
        private readonly IBaseService<MailAyar> _baseBusiness;

        // Constructor (yapıcı metod), dışarıdan bu sınıfa IBaseService<MailAyar> türünde bir bağımlılık (dependency) enjekte eder.
        // Dependency Injection (DI) prensibi sayesinde loosely-coupled (gevşek bağlı) bir yapı elde edilir.
        public MailAyarManager(IBaseService<MailAyar> baseBusiness)
        {
        ...
        public async Task<MailAyar> AddAsync(MailAyar entity)
        {
        ...
        public async Task<List<MailAyar>> GetAllAsync()
        {
        ...
        public async Task<MailAyar> GetByIdAsync(Guid id)
        {
        ...
        public async Task<MailAyar> UpdateAsync(MailAyar entity)
        {
