...
// SmsAyar adındaki Entity (veritabanı tablosunu temsil eden sınıf).
...
namespace TurkSoft.Service.Manager
{
    ...
    // ISmsAyarService arayüzünü (interface) implement ederek, bu interface'de tanımlanan tüm metodları somut olarak uygular.
    public class KeyAccountManager : IKeyAccountService
    {
        ...
        private readonly IBaseService<KeyAccount> _baseBusiness;

        // Constructor (yapıcı metod), dışarıdan bu sınıfa IBaseService<SmsAyar> türünde bir bağımlılık (dependency) enjekte eder.
        // Dependency Injection (DI) prensibi sayesinde loosely-coupled (gevşek bağlı) bir yapı elde edilir.
        public KeyAccountManager(IBaseService<KeyAccount> baseBusiness)
        {
        ...
        public async Task<KeyAccount> AddAsync(KeyAccount entity)
        {
        ...
        public async Task<List<KeyAccount>> GetAllAsync()
        {
        ...
        public async Task<KeyAccount> GetByIdAsync(Guid id)
        {
        ...
        public async Task<KeyAccount> UpdateAsync(KeyAccount entity)
        {
