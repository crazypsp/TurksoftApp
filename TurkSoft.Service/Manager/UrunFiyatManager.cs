...
// UrunFiyat adındaki Entity (veritabanı tablosunu temsil eden sınıf).
...
namespace TurkSoft.Service.Manager
{
    ...
    // IUrunFiyatService arayüzünü (interface) implement ederek, bu interface'de tanımlanan tüm metodları somut olarak uygular.
    public class UrunFiyatManager : IUrunFiyatService
    {
        ...
        private readonly IBaseService<UrunFiyat> _baseBusiness;

        // Constructor (yapıcı metod), dışarıdan bu sınıfa IBaseService<UrunFiyat> türünde bir bağımlılık (dependency) enjekte eder.
        // Dependency Injection (DI) prensibi sayesinde loosely-coupled (gevşek bağlı) bir yapı elde edilir.
        public UrunFiyatManager(IBaseService<UrunFiyat> baseBusiness)
        {
        ...
        public async Task<UrunFiyat> AddAsync(UrunFiyat entity)
        {
        ...
        public async Task<List<UrunFiyat>> GetAllAsync()
        {
        ...
        public async Task<UrunFiyat> GetByIdAsync(Guid id)
        {
        ...
        public async Task<UrunFiyat> UpdateAsync(UrunFiyat entity)
        {
