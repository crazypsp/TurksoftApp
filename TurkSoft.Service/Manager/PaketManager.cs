...
// Paket adındaki Entity (veritabanı tablosunu temsil eden sınıf).
...
namespace TurkSoft.Service.Manager
{
    ...
    // IPaketService arayüzünü (interface) implement ederek, bu interface'de tanımlanan tüm metodları somut olarak uygular.
    public class PaketManager : IPaketService
    {
        ...
        private readonly IBaseService<Paket> _baseBusiness;

        // Constructor (yapıcı metod), dışarıdan bu sınıfa IBaseService<Paket> türünde bir bağımlılık (dependency) enjekte eder.
        // Dependency Injection (DI) prensibi sayesinde loosely-coupled (gevşek bağlı) bir yapı elde edilir.
        public PaketManager(IBaseService<Paket> baseBusiness)
        {
        ...
        public async Task<Paket> AddAsync(Paket entity)
        {
        ...
        public async Task<List<Paket>> GetAllAsync()
        {
        ...
        public async Task<Paket> GetByIdAsync(Guid id)
        {
        ...
        public async Task<Paket> UpdateAsync(Paket entity)
        {
