...
{
    public sealed class TigerGlAccountRepository : ITigerGlAccountRepository
    {
        private readonly string _connectionString;

        public TigerGlAccountRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("TigerConnection");
        }

        public async Task<List<TigerGlAccount>> SearchAsync(string searchTerm, int take = 30)
        {
            var list = new List<TigerGlAccount>();
