import { OdemeService } from '../entities/OdemeService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await OdemeService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await OdemeService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await OdemeService.update(dto.Id, dto);
    else await OdemeService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
