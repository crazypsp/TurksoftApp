import { MailAyarService } from '../entities/MailAyarService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await MailAyarService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await MailAyarService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
    $$('.act-del').forEach(b => b.addEventListener('click', async e => {
      ...
      await MailAyarService.remove(id); 
      await loadTable();
    }));
  }
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await MailAyarService.update(dto.Id, dto);
    else await MailAyarService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
