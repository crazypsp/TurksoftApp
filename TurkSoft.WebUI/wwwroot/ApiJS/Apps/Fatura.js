import { FaturaService } from '../entities/FaturaService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await FaturaService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await FaturaService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await FaturaService.update(dto.Id, dto);
    else await FaturaService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
