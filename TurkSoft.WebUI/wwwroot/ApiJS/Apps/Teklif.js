import { TeklifService } from '../entities/TeklifService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await TeklifService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await TeklifService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await TeklifService.update(dto.Id, dto);
    else await TeklifService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
