import { UrunFiyatService } from '../entities/UrunFiyatService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await UrunFiyatService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await UrunFiyatService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
    $$('.act-del').forEach(b => b.addEventListener('click', async e => {
      ...
      await UrunFiyatService.remove(id); 
      await loadTable();
    }));
  }
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await UrunFiyatService.update(dto.Id, dto);
    else await UrunFiyatService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
