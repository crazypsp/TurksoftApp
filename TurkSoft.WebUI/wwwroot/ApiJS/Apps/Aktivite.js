import { AktiviteService } from '../entities/AktiviteService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await AktiviteService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await AktiviteService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
    $$('.act-del').forEach(b => b.addEventListener('click', async e => {
      ...
      await AktiviteService.remove(id); 
      await loadTable();
    }));
  }
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await AktiviteService.update(dto.Id, dto);
    else await AktiviteService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
