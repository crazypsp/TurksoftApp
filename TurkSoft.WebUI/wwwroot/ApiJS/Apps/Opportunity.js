import { OpportunityService } from '../entities/OpportunityService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await OpportunityService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await OpportunityService.update(dto.Id, dto);
    else await OpportunityService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
