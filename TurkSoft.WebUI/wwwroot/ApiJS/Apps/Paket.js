import { PaketService } from '../entities/PaketService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await PaketService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await PaketService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await PaketService.update(dto.Id, dto);
    else await PaketService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
