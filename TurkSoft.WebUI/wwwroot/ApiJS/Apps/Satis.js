import { SatisService } from '../entities/SatisService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await SatisService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await SatisService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await SatisService.update(dto.Id, dto);
    else await SatisService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
