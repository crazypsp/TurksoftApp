import { SmsAyarService } from '../entities/SmsAyarService.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadTable() {
    const list = await SmsAyarService.list();
  ...
  function bindRowActions() {
    $$('.act-edit').forEach(b => b.addEventListener('click', async e => {
      ...
      const r = await SmsAyarService.get(id);
      fmId.value = r.Id;
      modal?.show();
    }));
    $$('.act-del').forEach(b => b.addEventListener('click', async e => {
      ...
      await SmsAyarService.remove(id); 
      await loadTable();
    }));
  }
  ...
  formEl?.addEventListener('submit', async e => {
    ...
    if (dto.Id) await SmsAyarService.update(dto.Id, dto);
    else await SmsAyarService.create(dto);
    modal?.hide(); 
    await loadTable();
  });
